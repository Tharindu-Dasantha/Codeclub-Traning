# 🎨 Code Club Workshop: CSS Styling Magic!
> **Target Audience:** Grade 5-8 Students (Ages 10-14 in Sri Lanka)  
> **Duration:** 2 Hours (120 Minutes)  
> **Topic:** Introduction to CSS (Cascading Style Sheets) & Web Design  
> **Prerequisites:** Basic HTML knowledge (head, body, headings, paragraphs, lists, images)

---

## 📌 Workshop Overview

Welcome to **CSS Styling Magic**! This 2-hour interactive workshop is designed specifically for Sri Lankan Grade 5-8 students in Code Club. 

While HTML builds the skeleton of a webpage, **CSS (Cascading Style Sheets)** brings it to life with vibrant colors, custom fonts, rounded cards, shadows, and interactive hover animations. 

Through engaging analogies (like painting a house or choosing a fashion outfit), gamified bug hunting, live styling demonstrations, and a hands-on project (*Styling My Sri Lankan Explorer Page*), students learn how to make plain HTML look like a professional, modern website.

---

## 📁 Session Folder Structure

Everything required to facilitate or attend this session is located within this folder (`css-workshop-grade-5-8/`):

```text
css-workshop-grade-5-8/
├── README.md                   👈 You are here! Main overview & setup guide
├── LESSON_PLAN.md              👈 Detailed 120-minute facilitator step-by-step guide
├── STUDENT_CHEATSHEET.md       👈 Printable 1-page CSS property cheatsheet & Box Model guide
├── ACTIVITIES.md               👈 Step-by-step student activities, Bug Hunt challenge & Boss Quests
├── SLIDES.md                   👈 Slide-by-slide presentation script for projector projection
├── starter-kit/
│   ├── index.html              👈 Unstyled starter HTML page with class/ID hooks
│   └── style.css               👈 Guided starter stylesheet with comments for students
├── solution-kit/
│   ├── index.html              👈 Fully structured, production-ready solution HTML
│   └── style.css               👈 Complete, beautiful CSS stylesheet with animations & themes
└── interactive-demo/
    └── playground.html         👈 Zero-setup split-screen code editor & live preview in browser
```

---

## 🎯 Key Learning Objectives

By the end of this 2-hour session, students will be able to:
1. Explain what **CSS** is and how it differs from HTML (Skeleton vs Outfit/Paint).
2. Connect external stylesheets to HTML pages using the `<link>` tag.
3. Understand CSS selector types: **Element/Tag selectors** (`h1`), **Class selectors** (`.card`), and **ID selectors** (`#hero`).
4. Master core styling properties: `color`, `background-color`, `font-family`, `font-size`, `text-align`, `border`, `border-radius`, and `box-shadow`.
5. Understand the **CSS Box Model** (Content, Padding, Border, Margin).
6. Create interactive micro-animations using the `:hover` pseudo-class and `transition` property.
7. Debug common CSS errors (missing semicolons, incorrect selector prefixes, property typos).

---

## 🛠️ Required Setup & Hardware

### For Students:
* Desktop/Laptop computer running Windows, macOS, Linux, or Chromebook.
* **Code Editor:** VS Code, Notepad++, Sublime Text, or the included `interactive-demo/playground.html` in any web browser.
* **Web Browser:** Google Chrome, Mozilla Firefox, or Microsoft Edge.
* *No active internet connection required* during coding if using local starter files!

### For Instructor / Facilitator:
* Projector or large display screen.
* Whiteboard & color markers.
* Printed copies of `STUDENT_CHEATSHEET.md` (optional but recommended).

---

## 🚀 How to Run This Session

1. **Before Class (15 mins prior):**
   - Copy `starter-kit/` and `interactive-demo/` onto student machines or USB sticks.
   - Open `interactive-demo/playground.html` in Google Chrome to confirm offline functionality.
   - Review `LESSON_PLAN.md` for timing benchmarks.
2. **During Class:**
   - Follow the 120-minute timeline in `LESSON_PLAN.md`.
   - Project `SLIDES.md` on the main screen for visual explanation.
   - Facilitate Activity 1 (CSS Bug Hunt) and Activity 2 (Styling Explorer Page) from `ACTIVITIES.md`.
3. **After Class:**
   - Encourage students to show their colorful styled pages to classmates and parents!

---

*Created for Code Club Sri Lanka — Empowering future software engineers and web designers! 🇱🇰🎨💻*
